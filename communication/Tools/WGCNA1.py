...
from Plot import WGCNP
from Utilis import checkarray
import copy
...
class WGCN():
    ...
    def preprocess(self, adata=None,  cutHeight=float('inf'), save = None):
        ...
        """
        cf.log.CI(colored("Pre-processing...", 'blue'))
        adata = adata if not adata is None else self.adata

        obstree = cl.sci_linkage(adata.X, method="average",  metric='euclidean')
        clust = cl.sci_cuttree(obstree, height=cutHeight)
        index = np.where(clust[:,0] == 0)[0]
        cf.log.CI(colored(f"Keep {index.shape[0]} samples used for download analysis...", 'green'))
        WGCNP.sci_cluster(obstree, color_threshold=cutHeight, labels= adata.obs_names)

        self.adata = adata[index, :]
        cf.log.CI(colored("Done pre-processing..", 'blue'))

    def pickpower(self, adata = None, use_soft=True, **sotf_args ):
        cf.log.CI(colored("pick soft threshold...", 'blue'))
        adata = adata if not adata is None else self.adata

        if use_soft:
            kwgs = dict(RsquaredCut=self.RsquaredCut,
                        MeanCut=self.MeanCut,
                        powerVector=self.powers,
                        use_block=self.use_block,
                        networkType=self.networkType)
            kwgs.update(**sotf_args)
            self.power, self.sft = self.soft_power(adata.X, **kwgs)
        else:
            self.power = self.empirical_power(adata.var.shape[0], acttype='unsigned')


    def opt_power(self, dataMtx=None, **kargs):
        """
        pick soft threshold
        """
        print(f"{BOLD}{OKBLUE}pick soft threshold...{ENDC}")
        ...
        kwgs = dict( RsquaredCut=self.RsquaredCut,
                      ...
                      networkType=self.networkType)
        kwgs.update(**kargs)
        self.power, self.sft = WGCNA.pickSoftThreshold(self.adata.to_df(), **kwgs)
        ...
        ax[0].axhline(kwgs['RsquaredCut'], color='r')
    ...
    def power_corr(self,  **kargs):
        kwgs = dict(power=self.power, adjacencyType=self.networkType)
        kwgs.update(kargs)
        self.adjacency = WGCNA.adjacency(self.adata.to_df(), **kwgs)

    def power_TOM(self, **kargs):
        # Turn adjacency into topological overlap
        kwgs = dict(TOMType=self.TOMType)
        kwgs.update(kargs)
    
        self.TOM = WGCNA.TOMsimilarity(self.adjacency,**kwgs)
        self.dissTOM = (1 - self.TOM).round(decimals=8)

    def clusters(self, **kargs):
        ...
        dissTOM = self.dissTOM.copy()
        a = squareform(dissTOM.values, checks=False)
        # Call the hierarchical clustering function
        self.geneTree = linkage(a, method="average")
        # loc1 loc2 dist counts
        # Module identification using dynamic tree cut:
        self.dynamicMods = WGCNA.cutreeHybrid(dendro=self.geneTree, distM=dissTOM, **kwgs)
        ngroup = self.dynamicMods['Value'].unique().shape[0]
        print(colored( f"Identified {ngroup} modules using dynamic tree cut:", 'blue'))
        # Convert numeric labels into colors
        self.adata.var['dynamicColors'] = WGCNA.labels2colors(labels=self.dynamicMods)
    ...
    def scaleFreePower(self, k, nBreaks=10, base=1):
        """
    ...
    def soft_power(self, data= None, neighbor = None, RsquaredCut=0.9, MeanCut=100, Powers=None,
                          nBreaks=10, networkType="unsigned", cortype = 'pearson', 
                          use_empirical = False,
                          use_block=True, maxMemoryAllocation = 50 * 1024**3, blockSize=None):
        cf.log.CI(colored('find soft power threshod', 'blue'))
        Powers = list(range(1, 11)) + list(range(11, 21, 2)) if Powers is None else Powers
        powerSteps  = np.array(Powers)[1:] - np.array(Powers)[:-1]
        ...
        if not neighbor is None:
            ...
            if not data is None:
                ...
                data = checkarray(self.adata.X).copy()
            if cortype == 'pearson': 
                CORRX = np.corrcoef(data, rowvar=False)
            elif cortype == 'bicor': 
                CORRX = WGCNA.bicor(data)
            else:
        ...
        corx = WGCNA.activation(CORRX, acttype=networkType)

        if np.count_nonzero(np.isnan(corx)) != 0:
            cf.log.CW(colored(f"Some correlations are NA in matrix", 'red'))
        if any(np.diag(corx) < 0.999):
            cf.log.CW(colored(f"Some diagonal correlations are not 1 in matrix", 'red'))
            np.fill_diagonal(corx, 1)
        ...
        datout = [self.scaleFreePower(datk[:,i], nBreaks= nBreaks) for i in range(datk.shape[1]) ]
        datout = pd.concat(datout, axis=1).T
        datout['Power'] = Powers
        datout.index = Powers
        print(datout)
        # detect threshold more than 0.9 by default
        candidate = datout[((datout['SFT.R.sq'] > RsquaredCut) & (datout['mean(k)'] <= MeanCut))]
        if candidate.shape[0] > 0:
            powerEstimate = candidate['Power'].iloc[0]
            cf.log.CI(colored(f"Selected power to have scale free network is {powerEstimate}", 'green'))
        else:
            powerEstimate = datout.loc[ datout['SFT.R.sq'].idxmax(), 'Power' ]
            cf.log.CW(colored(f"No power detected to have scale free network!\nUse the max power given by R^2: {powerEstimate}", 'red'))
    ...
    def activation(X, acttype='unsigned', alpha=0.02):
        ...
        if acttype=='unsigned':
            ...
            'add tanh'
            print('Warning: invalid activate type. Will ignore activation.')
            return X
    ...
    def bicor_stand(x):
        ...
        if md==0:
            xb = x-np.mean(x)
            st = np.std(x) * np.sqrt(xb.shape[0]) #*1.4826
            return(xb/st)
        else:
            ux = (x -me)/(9*md)
    ...
    @staticmethod
    def adjacency(adata, selectCols=None, adjacencyType="unsigned", power=6, corOptions=pd.DataFrame(), weights=None,
                  weightArgNames=None):
        """
        Calculates (correlation or distance) network adjacency from given expression data or from a similarity

        :param adata: data frame containing expression data. Columns correspond to genes and rows to samples.
        :type adata: pandas dataframe
        ...
        weights = WGCNA.checkAndScaleWeights(weights, adata, scaleByMax=False)
        if weights is not None:
        ...
        if selectCols is None:
            cor_mat = np.corrcoef(
                adata.T)  # cor_mat = do.call(corFnc.fnc, c(list(x = adata), weightOpt, corOptions))
        else:
            cor_mat = np.corrcoef(x=adata, y=adata[:, selectCols])  # , weightOpt, corOptions)

        cor_mat = WGCNA.activation(cor_mat, acttype=adjacencyType)
        print("\tDone..\n")
