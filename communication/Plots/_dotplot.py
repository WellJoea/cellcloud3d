...
class DotPlot():
    ...
    def dot_plot(self, size_df, 
        ...
        if size_scale=='max':
            ...
            size_df = size_df - size_df.min().min()
        ...
        msize_df[msize_df==0] = np.nan
        scat = ax.scatter(xx, yy, s=msize_df, c=col_df, cmap=cmap, edgecolor=None, **kargs)
    ...
    def clustdot(self,
        ...
        if type(annot_linewidths) in [float, int] or len(annot_linewidths)==1:
            annot_linewidths = [annot_linewidths, annot_linewidths]
        if isinstance(annot_edgecolors, str) or len(annot_edgecolors)==1:
            annot_edgecolors = [annot_edgecolors, annot_edgecolors]
        annotkws = {} if annotkws is None else annotkws
        rannotkws = dict(linewidths=annot_linewidths[0],
                         edgecolor=annot_edgecolors[0],
                         **annotkws)
        cannotkws = dict(linewidths=annot_linewidths[1],
                         edgecolor=annot_edgecolors[1],
                         **annotkws)
        ##plot figure
        ...
        if (row_annot or is_add_row_annot):
            ...
            self.annot_plot(self.row_cor_df, self.ax_row_colors, **rannotkws)
        ...
        if (col_annot or is_add_col_annot):
            ...
            self.annot_plot(self.col_cor_df, self.ax_col_colors, swap_axes=True, **cannotkws)
