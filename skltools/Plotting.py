from sklearn.metrics import roc_curve, f1_score, precision_recall_curve, auc, average_precision_score, accuracy_score,confusion_matrix
...
class Baseset():
    def __init__(self, outfile, show=True, *array, **dicts):
        self.out = outfile
        self.show= show
...
class ROCPR(Baseset):
    ...
    def ROC_MImport(self, y_true, y_trueb, y_pred, y_proba, _model, _Y_name, show=True):
        n_labels  =[ re.sub('(_clf|_clf_c|_clf_mode|_clf_mean)$', '', i) for i in y_proba.columns ]
        n_classes = y_proba.shape[1]
        y_proba.columns  = n_labels
        color_ = self.color_*n_classes
        linestyle_ = self.linestyle_*n_classes
        #_color_ = color_[_n]( np.linspace(0.2, 0.8, n_classes) )

        _accuracy = accuracy_score(y_true, y_pred)
        confm = confusion_matrix(y_true, y_pred, labels=n_labels)
        _accuracies=confm.diagonal()/confm.sum(axis=1)
        for _n, _m in enumerate(n_labels):
            fpr, tpr, threshold_ = roc_curve(y_trueb[:,_n], y_proba[_m])
            roc_auci = auc(fpr, tpr)
            w=tpr-fpr
            ks_score=w.max()
            ks_x=fpr[w.argmax()]
            ...
            label_i = '%s: AUC (%0.4f), accuracy (%0.4f), KS (%0.4f)' %(_m, roc_auci, _accuracies[_n], ks_score)
            plt.plot(fpr, tpr, color=color_[_n], lw=1.8, linestyle=linestyle_[_n], label=label_i)
            plt.plot([ks_x,ks_x], [ks_x,ks_y],   lw=0.5, linestyle=linestyle_[_n], color=color_[_n], alpha=0.5)
            #plt.text(ks_x,(ks_x+ks_y)/2,'  KS=%.5f'%ks_score)
        ...
        plt.xlabel('False Positive Rate')
        plt.ylabel('True Positive Rate')
        plt.title( f'{_Y_name} {_model} ROC curve')
    ...
    def PR_MImport(self, y_true, y_trueb, y_pred, y_proba, _model, _Y_name, show=True):
        n_labels  =[ re.sub('(_clf|_clf_c|_clf_mode|_clf_mean)$', '', i) for i in y_proba.columns ]
        n_classes = y_proba.shape[1]
        y_proba.columns  = n_labels
        color_ = self.color_*n_classes
        ...
        f1_scores = f1_score(y_true, y_pred, average=None, labels=n_labels)
        for _n, _m in enumerate(n_labels):
            precision, recall, threshold_pr = precision_recall_curve(y_trueb[:,_n], y_proba[_m])
            average_precisioni = average_precision_score(y_trueb[:,_n], y_proba[_m])

            label_i = '%s: average precision (%0.4f), f1 score (%0.4f)' %(_m, average_precisioni, f1_scores[_n])
            plt.plot(recall, precision, color=color_[_n], lw=1.8, linestyle=linestyle_[_n], label=label_i)
        ...
        plt.xlabel('Recall')
        plt.ylabel('Precision')
        plt.title( f'{_Y_name} {_model} PR curve')
        leg = plt.legend(loc='center left', bbox_to_anchor=(1.0, 0.5), numpoints=1)
        plt.savefig( self.out, bbox_extra_artists=(leg,), bbox_inches='tight')
        if self.show:
            plt.show()

    def ROC_MImport0(self, ALL_Result, _model, _Y_name):
    ...
    def PR_MImport0(self, ALL_Result, _model, _Y_name):
        color_ = self.Sequential2*len(_model)
        linestyle_ = self.linestyle_*len(_model)
